...
def renderFrame(videosize, t):
    ...
    for element in elements:
        ...
        if t < element.start*1000 or t > element.end*1000 or element.type == "audio":
            continue
        if ENABLE_ERRORS:
            element.instance.render_start(t/1000 - element.start, videosize)
        else:
            try:
                element.instance.render_start(t/1000 - element.start, videosize)
...
class VideoPlayer(Rect):
    def __init__(
        ...
        app.event.add_listener(APPLY_PROPS, lambda: (
            self.draw(), app.refresh(self.rect)))
    ...
    def getVideoRect(self):
        ...
        topleft = (int(center[0] - scaledSize[0]//2),
                   int(center[1] - scaledSize[1]//2))
    ...
    def drawGridLines(self, frame, x, y, w, h):
        pg.draw.line(frame, (255, 255, 255, 100), (0, y+h/2),
                     (self.videosize[0], y+h/2), 10)
        pg.draw.line(frame, (255, 255, 255, 100), (x+w/2, 0),
                     (x+w/2, self.videosize[1]), 10)
    ...
    def update(self):
        ...
        if self.app.transformMode and self.isHoveredCurrently:
            ...
            for elementdata in self.elementData:
                # does element exist
                element = next(
                    (element for element in elements if element.id == elementdata[0]), None)
                if not element or not element.selected:
                    continue
                ...
                if self.app.mup:
                    ...
                    if element.dragging:
                        ...
                        element.dragging = inRect(
                            mx, my, *object_position, object_w, object_h)
                ...
                if element.dragging:
                    if self.app.holdingCtrl:
                        ...
                        # move based on grid
                        new_x, new_y = self.roundElementCenter(
                            mx, my, unit_x, unit_y, object_w, object_h)
                        element.x = new_x
                        element.y = new_y
                    else:
                        # move freely
    ...
    def drawContent(self):
        ...
        if not self.enabled:
            return
        if self.videosize == (0, 0):
            return
        if self.app.currentTime - self.lastRender < 30:
            return
        super().drawContent()
        ...
        for element in elements:
            # if element appears on the video
            if t < element.start*1000 or t > element.end*1000 or not element.type == "video":
                continue
            if ENABLE_ERRORS:
                element.instance.render_start(t/1000 - element.start, self.videosize)
            else:
                try:
                    element.instance.render_start(t/1000 - element.start, self.videosize)
                except Exception as e:
